...
export function OnboardingStepNav() {
  ...
  return (
    <div className="absolute left-0 top-0 bottom-0 z-10 flex min-h-0 w-full max-w-[18.75rem] flex-col justify-center py-[clamp(1rem,5vh,3rem)] pl-[clamp(1rem,4vw,2.5rem)] pr-0 max-lg:max-w-[16rem] max-lg:pl-6 max-md:relative max-md:max-w-none max-md:items-stretch max-md:px-4 max-md:-mb-12 max-md:pb-1 max-md:pt-1">
      <div className="olive-surface w-full relative isolate px-[26px] py-[30px] shadow-[0_16px_40px_rgba(0,0,0,0.24)] max-md:max-w-[calc(100vw-32px)] max-md:overflow-x-auto max-md:rounded-[22px] max-md:px-2.5 max-md:py-2">
        <ul
          ...
          {activeSteps.map((step, i) => {
            ...
            const stateClass = isDone
              ...
              : isActive
                ...
                : "";
            const rowClass = `onboarding-step-item ${stateClass} relative group flex min-h-[44px] w-full items-center gap-[18px] py-[15px] pr-2 max-lg:gap-4 max-md:h-11 max-md:w-11 max-md:min-w-[44px] max-md:justify-center max-md:rounded-[14px] max-md:border max-md:px-0 max-md:py-0 ${isDone ? "max-md:border-[rgba(122,154,58,0.2)] max-md:bg-[rgba(122,154,58,0.06)]" : isActive ? "max-md:border-[rgba(122,154,58,0.38)] max-md:bg-[rgba(122,154,58,0.12)]" : "max-md:border-[rgba(255,255,255,0.08)] max-md:bg-[rgba(255,255,255,0.02)]"} ${isClickable ? "onboarding-step-item--clickable m-0 h-auto cursor-pointer justify-start rounded-none border-none bg-transparent px-0 text-left text-inherit hover:bg-transparent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[rgba(168,196,98,0.78)] focus-visible:ring-offset-2 focus-visible:ring-offset-[rgba(8,10,14,0.92)] max-md:hover:border-[rgba(122,154,58,0.3)] max-md:hover:bg-[rgba(122,154,58,0.1)]" : "pointer-events-none max-md:pointer-events-auto"}`;
            ...
            if (isDone) {
              dotClass +=
                " !bg-[var(--onboarding-accent-bg,rgba(138,168,78,0.22))] !border-[var(--onboarding-accent-border-hover,rgba(168,196,98,0.4))]";
            } else if (isActive) {
              dotClass +=
                " !bg-accent !border-[rgba(220,235,200,0.9)] shadow-[0_0_12px_rgba(122,154,58,0.5)] animate-[onboarding-dot-pulse_2s_ease-in-out_infinite]";
            }

            // Name classes
            let nameClass =
              "onboarding-step-name text-[15px] tracking-[0.08em] font-medium transition-all duration-500";
            if (isDone) {
              nameClass +=
                " text-[var(--onboarding-nav-link,rgba(138,168,78,0.85))] drop-shadow-[0_1px_6px_rgba(3,5,10,0.42)] group-hover:text-[var(--onboarding-nav-link-hover,#a8c462)]";
            } else if (isActive) {
              nameClass +=
                " text-[var(--onboarding-nav-text-primary,#e8e8ec)] font-semibold drop-shadow-[0_1px_6px_rgba(3,5,10,0.42)]";
            } else {
              nameClass += " text-[#2a3318]";
            }

            // Subtitle classes
            let subClass =
              "onboarding-step-sub text-[13px] tracking-[0.05em] transition-all duration-500";
            if (isDone) {
              subClass +=
